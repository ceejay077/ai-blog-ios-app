...
import { generateArticle, generateSuggestions } from "@/lib/anthropic";
...
export default function CreateArticleScreen() {
  ...
  const [hasKeywordsInput, setHasKeywordsInput] = useState(false);
  ...
  const handleGenerateArticle = async () => {
    ...
    try {
      ...
      const wordCount = result.article.split(" ").length;

      if (!isPremium && wordCount > 500) {
        Alert.alert(
          "Word Limit Exceeded",
          `Your article is ${wordCount} words. Free users are limited to 500 words. Upgrade to Premium for unlimited words.`
        );
      }
  ...
  const handleShowSuggestions = async (field: "title" | "keywords") => {
    setSuggestionField(field);
    setLoading(true);
    const userInput = field === "title" ? title : keywords;
    const result = await generateSuggestions(category, field, userInput);
    setLoading(false);
        ...
        <View style={styles.form}>
          <View style={styles.inputContainer}>
            <View style={styles.labelRow}>
              ...
              <TouchableOpacity
                onPress={() => handleShowSuggestions("title")}
                disabled={!hasTitleInput}
              >
                <Text
                  style={[
                    styles.suggestionButton,
                    !hasTitleInput && styles.suggestionButtonDisabled,
                  ]}
                >
                  Suggest Title
                </Text>
            ...
            <TextInput
              ...
              onChangeText={(text) => {
                setTitle(text);
                setHasTitleInput(text.length > 0);
              }}
              multiline
            />
          </View>
          ...
          <View style={styles.inputContainer}>
            <View style={styles.labelRow}>
              ...
              <TouchableOpacity
                ...
                disabled={!hasKeywordsInput}
              >
                <Text
                  style={[
                    styles.suggestionButton,
                    !hasKeywordsInput && styles.suggestionButtonDisabled,
                  ]}
                >
                  Suggest Keywords
                </Text>
            ...
            <TextInput
              ...
              onChangeText={(text) => {
                setKeywords(text);
                setHasKeywordsInput(text.length > 0);
              }}
              multiline
            />
          </View>
